...
# Load the dataset
input_data = pd.read_csv('morongwa.csv')
#st.write(input_data)

# Generate future months and years
current_date = datetime.date(2023, 9, 30)  # Starting from April 2023
end_date = datetime.date(2025, 12, 30)  # Extend data up to December 2024
future_dates = pd.date_range(current_date, end_date, freq='M')

# Create a DataFrame with the generated future dates
future_data = pd.DataFrame({'year_month': future_dates.strftime('%Y-%m'), 'Month': future_dates})

# Concatenate the future data with the original data
input_data = pd.concat([input_data, future_data], ignore_index=True)
input_data['Month'] = pd.to_datetime(input_data['Month']) #converting the month column to a datetime format
#st.write(input_data)
...
if page_selection == "Overview":
    ...
    # Determine the available month options based on the selected year
    if selected_year == "2023":
        available_months = ["09", "10", "11", "12"]
    
    else:
        available_months = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12"]  # Show all months for 2024 and 2025
    ...
    meet_the_team()
        ...
        with st.form("Email Form"):
            ...
            if submit_res:
             ...
             st.markdown("Thank you for reaching out to us. We appreciate your interest in our CPI prediction web "
